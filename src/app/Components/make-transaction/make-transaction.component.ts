...
export class MakeTransactionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.transactionForm
      .get('productId')
      ?.valueChanges.subscribe(() => this.onProductChange());
  }

  onProductChange() {
    const selectedProduct = this.productss.find(
      (p) => p.id === this.transactionForm.value.productId
    );
    if (selectedProduct) {
      this.transactionForm.patchValue({ unit: selectedProduct.unit });
      console.log('Unit set to:', selectedProduct.unit);
  ...
  recordTransaction() {
    if (this.transactionForm.valid) {
      ...
      const productTransactionDto: ProductTransactionAddDto = {
        ...
        unit: this.transactionForm.value.unit,
        totalPrice: this.transactionForm.get('totalPrice')?.value,
      };
      console.log('Product Transaction DTO:', productTransactionDto);
